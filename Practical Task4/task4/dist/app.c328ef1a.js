...
var BASE_URL = 'https://www.gutenberg.org/cache/epub/29364/pg29364.txt';
var ReceivedText = "";
// Create an object to store the count of words starting with each letter
var letterCount = {};
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        ...
        case 3:
          ...
          text = response.data;
          ReceivedText = text;
          //console.log("Respose: ", ReceivedText);
          FindTheLetter(ReceivedText);
...
var FindTheLetter = function FindTheLetter(str) {
  var words = str.toLowerCase().split(" "); // split the text into word by spaces and convert all to lowercase

  mostCommonLetter = words[0][0]; // set common letter to the first character of first word
  highestCount = 1;
  // Loop through each word and count the number of words starting with each letter
  for (var i = 0; i < words.length; i++) {
    var word = words[i]; // get the first word
    var firstLetter = word[0]; // get the first letter of the word
    if (letterCount[firstLetter]) {
      letterCount[firstLetter]++; // increment count
    } else {
      letterCount[firstLetter] = 1; // set count to 1
    }
    // check if current letter count is greater then the highest count   
    if (letterCount[firstLetter] > highestCount) {
      mostCommonLetter = firstLetter; // replace most common letter
      highestCount = letterCount[firstLetter]; // replace highest count
    }
  }

  console.log("Rep: ", mostCommonLetter);
  console.log("count: ", highestCount);
  document.getElementById("letter").innerHTML = "Most Common Letter: " + " " + mostCommonLetter;
  document.getElementById("count").innerHTML = "Count of Letter: " + " " + highestCount;
};
//getTodoItems();
    ...
    return _regeneratorRuntime().wrap(function _callee2$(_context2) {
      while (1) switch (_context2.prev = _context2.next) {
        case 0:
          _context2.prev = 0;
          _context2.next = 3;
          return _axios.default.get("http://localhost:300/").then(function (response) {
            console.log;
            "Got: ", response;
          });
        case 3:
